...
replace_libs() { # $1 is $core_version $2 is $jqm_version $3 is $file
	...
	cat "$filename" | \
		sed -r "s@(http://code.jquery.com/jquery-)([0-9]+\.[0-9]+\.[0-9]+)@\1$core_version@g" | \
		sed -r "s@(http://code.jquery.com/mobile/)([^/]*)/jquery.mobile-(.*).min@\1$jqm_version/jquery.mobile-$jqm_version.min@g" \
		> "$tmpfile" &&
		mv "$tmpfile" "$filename"
	rm -f "$tmpfile"
}
